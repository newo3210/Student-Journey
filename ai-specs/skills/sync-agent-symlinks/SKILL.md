...
### Step 3b — Windows / symlink fallback (copy mode)

If creating a symlink fails (common on Windows without Developer Mode or admin):

1. Log: `symlink failed for <skill-name>, using copy-fallback`
2. **Physical copy** canonical folder to mirror target:
   - PowerShell: `Copy-Item -Recurse -Force ai-specs/skills/<skill-name> .cursor/skills/<skill-name>`
   - Same for `.claude/skills/` if syncing Claude mirror
3. For **agents** (single files), copy file instead of symlink:
   - `Copy-Item ai-specs/agents/<agent>.md .cursor/agents/<agent>.md`
4. Mark project state as `copy-fallback` in the sync report
5. Warn: changes in `ai-specs/` require re-sync to update copies

If symlink and copy both fail for critical skills (`project-director`, `enrich-us`, `adversarial-review`), ensure `.cursor/rules/sdd-orchestration.mdc` exists so the director is not deaf.
...
# Windows copy fallback (PowerShell)
Copy-Item -Recurse -Force ai-specs/skills/<skill-name> .cursor/skills/<skill-name>
