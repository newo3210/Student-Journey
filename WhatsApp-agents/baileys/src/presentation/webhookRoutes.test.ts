...
describe('webhook routes', () => {
  ...
  it('POST /webhook acknowledges and triggers humanized send for text', async () => {
  ...
  it('accepts matching x-webhook-secret when BAILEYS_WEBHOOK_SECRET is set', async () => {
    const sendPresence = vi.fn(async () => ({ status: 200, body: {} }))
    const sendMessage = vi.fn(async () => ({ status: 200, body: {} }))
    const app = createApp({
      env: { ...testEnv, BAILEYS_WEBHOOK_SECRET: 'shared-secret' },
      baileysClient: { sendPresence, sendMessage } as unknown as BaileysClient,
      sleep: async () => undefined,
    })
  ...
  it('does not send when fromMe is omitted on an otherwise valid payload', async () => {
    ...
    const res = await request(app)
      ...
      .send({
        event: 'message',
        payload: { from: '54911@s.whatsapp.net', id: 'BAILEYS.OMIT', body: 'coupon' },
      })
