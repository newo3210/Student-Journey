...
// Interactive reply ids - deterministic ids used if optional buttons ever fire.
...
// Demo flow options - coupon media URL and menu mode (text default).
...
// Resolve demo reply - maps keyword or interactive id to outbound messages.
...
// Build menu message - numbered text by default; buttons/list documented as unstable.
    ...
    case DEMO_BUTTON_IDS.info:
      return {
        messages: [
          buildTextMessage({
            to,
            body: 'This is the Baileys Level 1 demo: text, numbered menu (native buttons unstable), and media with presence+delay humanization.',
          }),
        ],
      }
    ...
    case DEMO_BUTTON_IDS.help:
      return {
        messages: [
          buildTextMessage({
            to,
            body: 'Keywords: menu | coupon | hi. Options: 1 Info, 2 Coupon PDF, 3 Help. Native buttons/list are unstable on Baileys; default is numbered text.',
          }),
        ],
      }
    default:
