...
// Optional webhook secret - empty string treated as unset (open simulator webhook).
...
// Env field schema - auth dir, port, coupon URL, menu mode, humanize delays.
export const envSchema = z
  .object({
    BAILEYS_AUTH_DIR: z.string().min(1).default('./auth_info_baileys'),
    PORT: z.coerce.number().int().positive().default(3003),
    COUPON_MEDIA_URL: z
      .string()
      .url()
      .default('https://www.w3.org/WAI/ER/tests/xhtml/testfiles/resources/pdf/dummy.pdf'),
    BAILEYS_MENU_MODE: z.enum(['text', 'buttons', 'list']).default('text'),
    BAILEYS_WEBHOOK_SECRET: optionalSecretSchema,
