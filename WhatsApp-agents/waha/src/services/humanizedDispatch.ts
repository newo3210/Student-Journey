...
import type { WahaClient, WahaSendResult } from '../infrastructure/wahaClient.js'
...
// Humanizer config - delay range, Waha client, optional sleep inject.
...
// Humanized dispatch - presence (typing) → stochastic delay → send. No Redis/BullMQ.
...
export async function humanizedSendAll(
  ...
  for (const message of messages) {
    ...
    if (result.sendStatus >= 200 && result.sendStatus < 300) {
      ...
      console.error('Waha send failed (non-2xx)', {
