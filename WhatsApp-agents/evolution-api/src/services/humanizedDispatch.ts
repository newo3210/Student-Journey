...
import type { EvolutionClient, EvolutionSendResult } from '../infrastructure/evolutionClient.js'
...
// Humanizer config - delay range, Evolution client, optional sleep inject.
...
// Humanized dispatch - presence (composing) → stochastic delay → send. No Redis/BullMQ.
...
export async function humanizedSendAll(
  ...
  for (const message of messages) {
    ...
    if (result.sendStatus >= 200 && result.sendStatus < 300) {
      ...
      console.error('Evolution send failed (non-2xx)', {
